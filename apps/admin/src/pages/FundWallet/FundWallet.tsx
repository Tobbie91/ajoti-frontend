  ...
  return (
    <div className="mx-auto w-full max-w-[600px] px-6 py-8">
      {step === 'form' && (
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white">
            <button
              onClick={() => { setShowBankDetails(!showBankDetails); setShowCards(false); setShowNewCardForm(false) }}
                ...
                <div className="rounded-xl bg-[#F9FAFB] p-4">
                  <div className="flex flex-col gap-3">
                    <div className="flex justify-between">
                      <Text fw={400} className="text-[13px] text-[#6B7280]">Bank Name</Text>
                      <Text fw={600} className="text-[13px] text-[#0F172A]">Wema Bank</Text>
                    </div>
                    <div className="flex justify-between">
                      <Text fw={400} className="text-[13px] text-[#6B7280]">Account Number</Text>
                      <Text fw={600} className="text-[13px] text-[#0F172A]">0124589632</Text>
                    </div>
                    <div className="flex justify-between">
                      <Text fw={400} className="text-[13px] text-[#6B7280]">Account Name</Text>
                      <Text fw={600} className="text-[13px] text-[#0F172A]">Ajoti/Admin Account</Text>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white">
            <button
              onClick={() => { setShowCards(!showCards); setShowBankDetails(false); setShowNewCardForm(false) }}
              className="flex w-full cursor-pointer items-center justify-between p-5"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <Text fw={600} className="text-[14px] text-[#0F172A]">Select Card</Text>
                  <Text fw={400} className="text-[12px] text-[#6B7280]">
                    {selectedCard ? `**** ${savedCards.find((c) => c.id === selectedCard)?.last4}` : 'Choose a saved card'}
                  </Text>
                </div>
              </div>
              {showCards ? <IconChevronUp size={18} color="#9CA3AF" /> : <IconChevronDown size={18} color="#9CA3AF" />}
            </button>
            {showCards && (
              <div className="border-t border-[#E5E7EB] px-5 pb-5 pt-4">
                <Text fw={500} className="mb-3 text-[12px] text-[#6B7280]">Saved Cards</Text>
                <div className="flex flex-col gap-2">
                  {savedCards.map((card) => (
                    <button
                      key={card.id}
                      onClick={() => { setSelectedCard(card.id); setShowCards(false) }}
                      className={`flex w-full cursor-pointer items-center gap-3 rounded-xl border p-3 transition-colors ${
                        selectedCard === card.id ? 'border-[#02A36E] bg-[#F0FDF4]' : 'border-[#E5E7EB] bg-white hover:bg-[#F9FAFB]'
                      }`}
                    >
                      ...
                      <div className="flex-1 text-left">
                        <Text fw={600} className="text-[13px] text-[#0F172A]">{card.brand} •••• {card.last4}</Text>
                        <Text fw={400} className="text-[11px] text-[#9CA3AF]">Expires {card.expiry}</Text>
                      </div>
                  ...
                  {!showNewCardForm ? (
                    ...
                    >
                      ...
                      <Text fw={500} className="text-[13px] text-[#6B7280]">Add New Card</Text>
                    </button>
                  ) : (
                    <div className="rounded-xl border border-[#02A36E] bg-[#F0FDF4] p-4">
                      <Text fw={600} className="mb-3 text-[13px] text-[#0F172A]">New Card Details</Text>
                      <div className="flex flex-col gap-3">
                        <TextInput
                          ...
                          onChange={(e) => {
                            ...
                            setNewCardNumber(digits.replace(/(.{4})/g, '$1 ').trim())
                          }}
                        ...
                        <div className="flex gap-3">
                          ...
                          <TextInput
                            ...
                            size="sm"
                            value={newCardCvv}
                            onChange={(e) => setNewCardCvv(e.currentTarget.value.replace(/\D/g, '').slice(0, 3))}
                            styles={{ input: { borderColor: '#E5E7EB', fontSize: 13 } }}
                            className="flex-1"
                          />
                        </div>
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (digits.length === 16 && newCardExpiry.length === 5 && newCardCvv.length === 3) {
                                ...
                                setSavedCards((prev) => [...prev, { id: newId, last4: digits.slice(-4), brand: 'Card', expiry: newCardExpiry }])
                                setSelectedCard(newId)
                                setNewCardNumber(''); setNewCardExpiry(''); setNewCardCvv(''); setShowNewCardForm(false)
                              }
                            }}
                            disabled={newCardNumber.replace(/\s/g, '').length !== 16 || newCardExpiry.length !== 5 || newCardCvv.length !== 3}
                            className={`flex-1 rounded-lg py-2 text-[13px] font-semibold text-white ${
                              newCardNumber.replace(/\s/g, '').length === 16 && newCardExpiry.length === 5 && newCardCvv.length === 3
                                ? 'cursor-pointer bg-[#02A36E] hover:bg-[#028a5b]' : 'cursor-not-allowed bg-[#9CA3AF]'
                            }`}
                          >
                            Add Card
                          </button>
                          <button
                            onClick={() => { setShowNewCardForm(false); setNewCardNumber(''); setNewCardExpiry(''); setNewCardCvv('') }}
          ...
          <button
            onClick={() => { if (numericAmount > 0 && selectedCard) setStep('confirm') }}
            disabled={numericAmount <= 0 || !selectedCard}
            className={`w-full rounded-xl py-3.5 text-[14px] font-semibold text-white ${
              numericAmount > 0 && selectedCard ? 'cursor-pointer bg-[#02A36E] hover:bg-[#028a5b]' : 'cursor-not-allowed bg-[#9CA3AF]'
            }`}
          >
            Proceed
          </button>
      ...
      {(step === 'confirm' || step === 'processing') && (
        <div className="flex flex-col gap-6">
          {step === 'confirm' && (
            <button onClick={() => setStep('form')} className="mb-2 flex cursor-pointer items-center gap-2 text-[14px] font-medium text-[#374151] hover:text-[#0F172A]">
              <IconArrowLeft size={18} /> Back
            </button>
          )}
          <div>
            <Text fw={700} className="text-[24px] text-[#0F172A]">Confirm Payment</Text>
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-[#F3F4F6] pb-4">
                <Text fw={400} className="text-[14px] text-[#6B7280]">Amount</Text>
                <Text fw={700} className="text-[18px] text-[#0F172A]">₦{amount}</Text>
              </div>
              <div className="flex items-center justify-between border-b border-[#F3F4F6] pb-4">
                <Text fw={400} className="text-[14px] text-[#6B7280]">Payment Method</Text>
                <Text fw={600} className="text-[14px] text-[#0F172A]">•••••{savedCards.find((c) => c.id === selectedCard)?.last4}</Text>
              </div>
              <div className="flex items-center justify-between border-b border-[#F3F4F6] pb-4">
                <Text fw={400} className="text-[14px] text-[#6B7280]">Transaction Fee</Text>
                <Text fw={500} className="text-[14px] text-[#0F172A]">₦{transactionFee.toLocaleString()}</Text>
              </div>
              <div className="flex items-center justify-between">
                <Text fw={600} className="text-[14px] text-[#0F172A]">Total Charge</Text>
                <Text fw={700} className="text-[18px] text-[#02A36E]">₦{totalCharge.toLocaleString()}</Text>
              </div>
            </div>
          </div>
          {step === 'confirm' && (
        ...
        <div className="flex flex-col items-center gap-8 pt-8">
          ...
          <div className="grid w-[280px] grid-cols-3 gap-3">
            {['1','2','3','4','5','6','7','8','9','','0','back'].map((key) => {
              ...
              if (key === 'back') return (
                <button key="back" onClick={handlePinBackspace} className="flex h-16 cursor-pointer items-center justify-center rounded-2xl bg-[#F3F4F6] hover:bg-[#E5E7EB]">
      ...
      {step === 'success' && (
        <div className="flex flex-col items-center gap-6 pt-16">
          ...
          <div className="text-center">
            <Text fw={700} className="text-[22px] text-[#0F172A]">Payment Successful</Text>
            <Text fw={400} className="mt-2 max-w-[320px] text-[14px] leading-[1.6] text-[#6B7280]">₦{amount} has been added to your NGN wallet</Text>
          </div>
          <button onClick={() => navigate('/dashboard')} className="mt-4 w-full max-w-[300px] cursor-pointer rounded-xl bg-[#02A36E] py-3.5 text-[14px] font-semibold text-white hover:bg-[#028a5b]">
            Done
          </button>
        </div>
      )}
