...
export function GroupDetails() {
  ...
  const [reviewStep, setReviewStep] = useState<"form" | "submitting" | "done">(
    "form",
  );
      ...
      try {
        ...
        getCircleRules()
          .then((res) =>
            setPostStartExitPenaltyPercent(
              res.data.postStartExitPenaltyPercent,
            ),
          )
        ...
        if (circle) {
          ...
          setGroup({
            id: circle.id,
            name: circle.name,
            status: (circle.visibility === "PRIVATE"
              ? "Invite Only"
              : "Open") as GroupStatus,
    ...
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3">
        ...
        <button
          ...
          className="text-[#02A36E] cursor-pointer text-sm font-medium"
        >
          Back to groups
        </button>
      </div>
  ...
  const canLeave =
    isMember && (circleStatus === "DRAFT" || circleStatus === "ACTIVE");
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-5">
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6">
            ...
            <Text
              fw={400}
              className="mt-4 text-[13px] leading-relaxed text-[#6B7280]"
            >
              {group.adminBio}
            </Text>
          </div>
              ...
              <Table.Tbody>
                {schedule.map((row) => (
                  <Table.Tr key={row.month}>
                    <Table.Td>
                      <Text fw={500} className="text-[#0F172A]">
                        {row.month}
                      </Text>
                    </Table.Td>
                    <Table.Td>
              ...
              <>
                ...
                <div className="mt-4 flex items-center gap-3">
                  <Avatar size={44} radius="xl" color="teal" variant="filled">
                    {(
                      reviewTarget.name ||
                      `${reviewTarget.firstName ?? ""} ${reviewTarget.lastName ?? ""}`.trim()
                    )
                      .slice(0, 2)
                      .toUpperCase()}
                  </Avatar>
                  <div>
                    <Text fw={600} className="text-[15px] text-[#0F172A]">
                      {reviewTarget.name ||
                        `${reviewTarget.firstName ?? ""} ${reviewTarget.lastName ?? ""}`.trim() ||
                        `User ${reviewTarget.userId.slice(0, 6)}`}
                    </Text>
                ...
                <div className="mt-6">
                  ...
                  {reviewRating > 0 && (
                    <Text
                      fw={500}
                      className="mt-2 text-center text-[12px] text-[#6B7280]"
                    >
                      {
