    ...
    <div className="mx-auto w-full max-w-[480px] px-6 py-12">
      ...
      {state === "loading-preview" && (
        <div className="flex flex-col items-center rounded-2xl border border-[#E5E7EB] bg-white px-8 py-16">
          ...
          <Text fw={500} className="mt-4 text-[14px] text-[#6B7280]">
      ...
      {state === "confirm" && preview && (
        <div className="flex flex-col items-center rounded-2xl border border-[#E5E7EB] bg-white px-8 py-10">
          ...
          <Text
            fw={700}
            className="mt-6 text-center text-[22px] text-[#0F172A]"
          >
            You've been invited!
          </Text>
          <Text
          ...
          >
            <span className="font-semibold text-[#0F172A]">
              {preview.circle.adminName}
            </span>{" "}
            invited you to join their savings circle
          </Text>
          ...
          <div className="mt-6 w-full rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] px-5 py-4">
            ...
            <div className="mt-3 flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <Text fz={13} fw={600} style={{ color: PRIMARY }}>
                  {formatNaira(preview.circle.contributionAmount)}
                </Text>
              </div>
              <div className="flex items-center justify-between">
                <Text fz={13} c="dimmed">
                ...
                <Text fz={13} fw={500}>
                  {formatFreq(preview.circle.frequency)}
                </Text>
              </div>
              <div className="flex items-center justify-between">
                <Text fz={13} c="dimmed">
                ...
                <Text fz={13} fw={500}>
                  {preview.circle.durationCycles} cycles
                </Text>
              </div>
              <div className="flex items-center justify-between">
                <Text fz={13} c="dimmed">
                ...
                <Text fz={13} fw={500}>
                  {preview.circle.filledSlots} / {preview.circle.maxSlots}{" "}
                  filled
                </Text>
              </div>
            </div>
          ...
          <div className="mt-3 flex w-full items-center gap-2 rounded-lg bg-[#FFFBEB] px-4 py-2.5">
            ...
            <Text fz={12} style={{ color: "#92400E" }}>
              Invite expires on <strong>{expiryStr}</strong>
            </Text>
          </div>

          {/* Before you accept */}
          <div className="mt-4 w-full rounded-xl border border-[#FEF3C7] bg-[#FFFBEB] px-5 py-4">
            <Text fw={600} className="text-[13px] text-[#92400E]">
              Before you accept
            </Text>
      ...
      {state === "submitting" && (
        <div className="flex flex-col items-center rounded-2xl border border-[#E5E7EB] bg-white px-8 py-16">
          <Loader color={PRIMARY} size="lg" />
          <Text fw={700} className="mt-6 text-[18px] text-[#0F172A]">
            Accepting invite…
          </Text>
          <Text fw={400} className="mt-1 text-[13px] text-[#6B7280]">
            Reserving your collateral
          </Text>
        </div>
      )}
      ...
      {state === "success" && (
        <div className="flex flex-col items-center rounded-2xl border border-[#E5E7EB] bg-white px-8 py-10">
          ...
          {preview && (
            <Text
              fw={400}
              className="mt-2 text-center text-[14px] text-[#6B7280]"
            >
          ...
          <Text
            fw={400}
            className="mt-1 max-w-[300px] text-center text-[13px] text-[#9CA3AF]"
          >
            Your collateral has been reserved. You'll be notified once the
            circle starts.
          </Text>
          <button
            onClick={() => navigate("/rosca")}
      ...
      {state === "error" && (
        <div className="flex flex-col items-center rounded-2xl border border-[#E5E7EB] bg-white px-8 py-10">
          ...
          <Text
            fw={400}
            className="mt-2 max-w-[320px] text-center text-[14px] leading-relaxed text-[#6B7280]"
          >
            {errorMessage ||
              "This invite may have expired, already been used, or was sent to a different email."}
          </Text>
