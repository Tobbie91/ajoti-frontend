...
const ICON_MAP: Record<string, (size: number) => React.ReactNode> = {
  ...
  "calendar-event": (s) => (
    <IconCalendarEvent size={s} color="white" stroke={1.5} />
  ),
  "shield-check": (s) => (
    <IconShieldCheck size={s} color="white" stroke={1.5} />
  ),
  "shield-check-green": (s) => (
    <IconShieldCheck size={s} color="white" stroke={1.5} />
  ),
  "alert-triangle": (s) => (
    <IconAlertTriangle size={s} color="white" stroke={1.5} />
  ),
...
function getIcon(article: Article, size = 48) {
  ...
  return renderer ? (
    ...
    <IconHelpCircle size={size} color="white" stroke={1.5} />
  );
}
    ...
    case "heading":
      return (
        <Text
          key={index}
          fw={700}
          className="mt-10 text-[20px] text-[#0F172A] first:mt-0"
        >
          {block.text}
        </Text>
      );
    ...
    case "paragraph":
      return (
        <Text
          key={index}
          fw={400}
          className="mt-4 text-[15px] leading-[1.9] text-[#374151]"
        >
          {block.text}
        </Text>
      );
      ...
      return (
        <ul key={index} className="mt-4 flex flex-col gap-3 pl-5">
          {block.items.map((item, i) => (
            <li
              key={i}
              className="list-disc text-[15px] leading-[1.8] text-[#374151]"
            >
              {item}
            </li>
          ))}
      ...
      return (
        <ol key={index} className="mt-4 flex flex-col gap-3 pl-5">
          {block.items.map((item, i) => (
            <li
              key={i}
              className="list-decimal text-[15px] leading-[1.8] text-[#374151]"
            >
              {item}
            </li>
          ))}
    ...
    case "quote":
      return (
        ...
        >
          <Text
            fw={500}
            className="italic text-[15px] leading-[1.8] text-[#374151]"
          >
            {block.text}
          </Text>
        </div>
    ...
    <div>
      ...
      >
        <div className="mx-auto max-w-[760px] px-4 py-10 sm:px-6 sm:py-16">
          ...
          <Text
            fw={800}
            className="mt-4 text-[22px] leading-snug text-white sm:max-w-[600px] sm:text-[32px]"
          >
            {article.title}
          </Text>
          ...
          <Text
            fw={400}
            className="mt-3 text-[14px] leading-relaxed text-white/80 sm:max-w-[520px] sm:text-[15px]"
          >
            {article.excerpt}
          </Text>
        </div>
              ...
              {related.map((rel) => (
                ...
                >
                  ...
                  <div className="flex-1 p-4">
                    <Text
                      fw={600}
                      className="text-[13px] leading-snug text-[#0F172A]"
                    >
                      {rel.title}
                    </Text>
