...
export function JoinSummary() {
  ...
  const deadlineRaw = (circle as Record<string, unknown>)
    .initialContributionDeadline as string | null | undefined;
  const firstPaymentDate = deadlineRaw ? new Date(deadlineRaw) : null;
  const lastPaymentDate = firstPaymentDate
    ? addPeriods(firstPaymentDate, frequency, duration - 1)
    : null;
      ...
      <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <Text fw={500} className="text-[13px] text-[#6B7280]">
                Contribution per {frequencyLabel[frequency] ?? "cycle"}
              </Text>
              <Text fw={600} className="text-[14px] text-[#0F172A]">
                ₦
                {contributionNaira.toLocaleString("en-NG", {
            ...
            <div className="flex items-center justify-between">
              <Text fw={500} className="text-[13px] text-[#6B7280]">
                Total Payout (when your turn comes)
              </Text>
              <Text fw={600} className="text-[14px] text-[#0F172A]">
                ₦
                {totalToRaise.toLocaleString("en-NG", {
                  minimumFractionDigits: 2,
                })}
              </Text>
            </div>
        ...
        {hasInsufficientBalance && (
          <div className="flex items-start gap-3 rounded-xl bg-[#FEF2F2] px-5 py-4">
            ...
            <Text
              fw={500}
              className="text-[13px] leading-relaxed text-[#991B1B]"
            >
              Insufficient wallet balance. Please fund your wallet before
              proceeding.
            </Text>
          </div>
        )}
