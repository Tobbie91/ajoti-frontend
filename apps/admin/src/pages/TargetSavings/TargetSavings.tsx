...
const money = (k: string) =>
  `₦${toNaira(k).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`;
    ...
    <div className="mx-auto w-full max-w-[1100px] px-4 py-6">
      <Group justify="space-between" mb="lg">
        <div>
          ...
          <Text c="dimmed">
            Save towards your own goal or stay accountable with a group.
          </Text>
        </div>
        <Button
        ...
        {plans.length === 0 && (
          <Card withBorder>
            ...
            <Text size="sm" c="dimmed">
              Create an individual target or start a group target.
            </Text>
          </Card>
        )}
        {plans.map((p) => (
                ...
                <Group justify="space-between">
                  <div>
                    ...
                    <Text size="sm" c="dimmed">
                      ...
                      {money(p.contributionAmountKobo)}{" "}
      ...
      >
        <Stack>
          ...
          <Button
            loading={busy}
            disabled={!form.name || !form.maturityDate}
            onClick={create}
          >
            Create target
          </Button>
...
function TargetCard({
  ...
  plan: TargetSavingsPlan;
  onChanged: () => Promise<unknown>;
}) {
  const [amount, setAmount] = useState(toNaira(plan.contributionAmountKobo));
    ...
    <Card withBorder radius="md" p="lg">
      <Group justify="space-between" align="flex-start">
        <div>
          ...
          <Text size="sm" c="dimmed">
            {plan.type === "GROUP"
              ? `${plan.memberCount} members · Group accountability`
              : "Individual target"}{" "}
      ...
      <Group justify="space-between" mt="xs">
        ...
        <Text size="sm">Goal {money(plan.targetAmountKobo)}</Text>
      </Group>
      {plan.status === "ACTIVE" && mine && (
        <Group mt="md">
          <NumberInput
            min={1}
            value={amount}
          ...
          {plan.members
            ...
            .map((m, i) => (
              <Group key={m.id} justify="space-between">
                <Text size="sm">
                  {i + 1}. {m.user.firstName} {m.user.lastName}
                </Text>
