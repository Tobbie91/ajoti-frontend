...
export function Sidebar({ onClose }: SidebarProps) {
  ...
  const [savingsOpened, setSavingsOpened] = useState(location.pathname.startsWith('/savings'))
  ...
  return (
    <Box style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box p="md" mb="md" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
        <Text fw={800} fz={22} c="primary.5">Ajoti</Text>
      ...
      <ScrollArea style={{ flex: 1 }} type="auto" scrollbarSize={6} offsetScrollbars>
        <Stack gap={4} px="xs" pb="md">
          {visibleMain.map((link) => (
            <NavLink key={link.path} component={RouterNavLink} to={link.path} label={link.label}
              leftSection={<link.icon size={20} stroke={1.5} />} active={location.pathname === link.path} onClick={onClose} />
          ))}
          <NavLink label="Savings" leftSection={<IconPigMoney size={20} stroke={1.5} />} childrenOffset={28}
            opened={savingsOpened} onChange={() => setSavingsOpened((o) => !o)} active={location.pathname.startsWith('/savings')}>
            {savingsChildren.map((child) => (
              <NavLink key={child.path} component={RouterNavLink} to={child.path}
                label={<span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{child.label}{child.comingSoon && <Badge size="xs" variant="light" color="gray">Soon</Badge>}</span>}
                leftSection={<child.icon size={18} stroke={1.5} />} active={location.pathname === child.path} onClick={onClose} />
            ))}
          </NavLink>
