  ...
  return (
    <Stack gap="md">
      <Group gap="xs">
        ...
        <Stack gap={2}>
          <Text fw={600} fz="lg">{user.firstName as string} {user.lastName as string}</Text>
      ...
      {kyc && (
        <Button
          ...
          onClick={() => window.location.assign(`/kyc-approvals?userId=${encodeURIComponent(user.id as string)}`)}
        >
          View KYC
        </Button>
      )}
      ...
      <Group gap="xs">
        ...
        {wallet && <Badge ml="auto" size="xs" color={wallet.status === 'ACTIVE' ? 'green' : 'gray'} variant="light">{wallet.status}</Badge>}
      </Group>

      <SimpleGrid cols={2} spacing="xs">
        <Card withBorder p="sm" radius="md">
          <Group gap="xs">
            <ThemeIcon size={28} radius="md" variant="light" color="grape"><IconTopologyRing size={14} stroke={1.5} /></ThemeIcon>
            <Stack gap={0}><Text fz="xs" c="dimmed">ajo Groups</Text><Text fw={600} fz="sm">{roscaCount}</Text></Stack>
          </Group>
        </Card>
        <Card withBorder p="sm" radius="md">
          <Group gap="xs">
            <ThemeIcon size={28} radius="md" variant="light" color={debtCount > 0 ? 'red' : 'green'}><IconShieldCheck size={14} stroke={1.5} /></ThemeIcon>
            <Stack gap={0}><Text fz="xs" c="dimmed">Outstanding Debts</Text><Text fw={600} fz="sm" c={debtCount > 0 ? 'red' : undefined}>{debtCount}</Text></Stack>
          </Group>
        </Card>
      </SimpleGrid>
      ...
      {(user.adminRequestedAt as string | null) && hasPermission(getStaffRoleFromStorage(), 'MANAGE_ADMIN_ACCOUNTS') && (
        <Alert color="orange" radius="md" variant="light" title="Pending Admin Request">
          ...
          <Group mt="xs" gap="xs">
            <Button size="xs" color="green" variant="filled" onClick={onOpenApproveAdmin} loading={actionLoading}>Approve</Button>
            <Button size="xs" color="red" variant="light" onClick={onOpenRejectAdmin} loading={actionLoading}>Reject</Button>
          </Group>
        </Alert>
      )}
      ...
      {(() => {
        ...
        if (!canSuspendAccount && !canManageAdminAccounts) return null
              ...
              <>
                {currentStatus === 'FROZEN' && (
                  <Alert color="orange" radius="md" variant="light" title="Self-Frozen Account">
                    ...
                    <Button fullWidth mt="xs" variant="filled" color="orange" onClick={onUnfreeze} loading={actionLoading}>
                      Unfreeze Account (after identity verification)
                    </Button>
                  </Alert>
                )}
                {currentStatus !== 'ACTIVE' && currentStatus !== 'FROZEN' && (
                  <Button fullWidth variant="light" color="green" onClick={onReactivate} loading={actionLoading}>Reactivate Account</Button>
                )}
                {currentStatus === 'ACTIVE' && <Button fullWidth variant="light" color="yellow" onClick={onOpenSuspend}>Suspend Account</Button>}
                {currentStatus !== 'BANNED' && <Button fullWidth variant="light" color="red" onClick={onOpenBan}>Ban Account</Button>}
              </>
            )}
          </Stack>
        )
...
function UserDetailDrawer({ userId, opened, onClose, onStatusChange }: {
  userId: string | null
  opened: boolean
  onClose: () => void
  onStatusChange: () => void
  ...
  async function handleStatus(status: 'ACTIVE' | 'SUSPENDED' | 'BANNED', reasonText?: string) {
    ...
    try {
      ...
      closeSuspend(); closeBan(); onStatusChange(); onClose()
  ...
  async function handleUnfreeze() {
    if (!userId) return
    setActionLoading(true)
    try { await unfreezeAccount(userId); onStatusChange(); onClose() }
    catch (e) { setError(e instanceof Error ? e.message : 'Action failed') }
    finally { setActionLoading(false) }
  }
  ...
  async function handleApproveAdmin() {
    if (!userId) return
    setActionLoading(true)
    try { await approveAdminRequest(userId); closeApproveAdmin(); onStatusChange(); onClose() }
  ...
  async function handleRejectAdmin() {
    if (!userId) return
    setActionLoading(true)
    try { await rejectAdminRequest(userId); closeRejectAdmin(); onStatusChange(); onClose() }
  ...
  return (
    <>
      <Drawer opened={opened} onClose={onClose} title={<Text fw={600}>User Profile</Text>} position="right" size="md" padding="lg">
        {loading ? (
          <Stack gap="sm">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} h={40} radius="sm" />)}</Stack>
        ) : error ? (
      ...
      <Modal opened={suspendModal} onClose={closeSuspend} title="Suspend User" size="sm">
        <Stack gap="md">
          ...
          <Textarea placeholder="Reason for suspension..." value={reason} onChange={(e) => setReason(e.currentTarget.value)} rows={3} />
          <Group justify="flex-end">
            <Button variant="default" onClick={closeSuspend}>Cancel</Button>
            <Button color="yellow" loading={actionLoading} onClick={() => { handleStatus('SUSPENDED', reason || undefined); setReason('') }}>Suspend</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      <Modal opened={banModal} onClose={closeBan} title="Ban User" size="sm">
        <Stack gap="md">
          ...
          <Textarea placeholder="Reason for ban..." value={reason} onChange={(e) => setReason(e.currentTarget.value)} rows={3} />
          <Group justify="flex-end">
            <Button variant="default" onClick={closeBan}>Cancel</Button>
            <Button color="red" loading={actionLoading} onClick={() => { handleStatus('BANNED', reason || undefined); setReason('') }}>Ban</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      <Modal opened={approveAdminModal} onClose={closeApproveAdmin} title="Approve Admin Request" size="sm">
        <Stack gap="md">
          <Text fz="sm" c="dimmed">This will grant admin access to <strong>{user?.firstName as string} {user?.lastName as string}</strong> and allow them to create and manage ajo groups.</Text>
          <Group justify="flex-end"><Button variant="default" onClick={closeApproveAdmin}>Cancel</Button><Button color="green" loading={actionLoading} onClick={handleApproveAdmin}>Approve</Button></Group>
        </Stack>
      </Modal>

      <Modal opened={rejectAdminModal} onClose={closeRejectAdmin} title="Reject Admin Request" size="sm">
        <Stack gap="md">
          <Text fz="sm" c="dimmed">This will reject the admin access request from <strong>{user?.firstName as string} {user?.lastName as string}</strong>. They will remain a regular member.</Text>
          <Group justify="flex-end"><Button variant="default" onClick={closeRejectAdmin}>Cancel</Button><Button color="red" loading={actionLoading} onClick={handleRejectAdmin}>Reject</Button></Group>
        </Stack>
      </Modal>
    </>
  )
...
function InfoItem({ label, value }: { label: string; value: ReactNode }) {
  return <Stack gap={2}><Text fz="xs" c="dimmed">{label}</Text>{typeof value === 'string' ? <Text fz="sm" fw={500}>{value || '-'}</Text> : value}</Stack>
}
...
export function ManageUsers() {
  ...
  function fetchUsers() {
    ...
    listUsers({
      ...
      .then((res) => { setUsers(res.data); setTotal(res.meta.total); setTotalPages(res.meta.totalPages) })
  ...
  function openUserDetail(userId: string) { setSelectedUserId(userId); openDrawer() }

  function handleFiltersReset() {
    setSearch(''); setRoleFilter(null); setStatusFilter(null); setKycFilter(null); setPendingAdminFilter(false); setPage(1)
  }
  ...
  return (
    <Stack gap="md">
      <Group justify="space-between" align="center"><Title order={2}>Manage Users</Title><Text fz="sm" c="dimmed">{total.toLocaleString()} users total</Text></Group>
          ...
          <Table verticalSpacing="sm" horizontalSpacing="md" layout="fixed">
            <Table.Thead>
              <Table.Tr style={{ backgroundColor: '#0B6B55' }}>
                ...
                <Table.Th style={{ color: 'white' }} w={90} />
              </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => <Table.Tr key={i}>{Array.from({ length: 9 }).map((__, j) => <Table.Td key={j}><Skeleton h={20} radius="sm" /></Table.Td>)}</Table.Tr>)
              ) : users.length === 0 ? (
                <Table.Tr><Table.Td colSpan={9}><Text ta="center" py="xl" c="dimmed">{hasFilters ? 'No users match your filters' : 'No users yet'}</Text></Table.Td></Table.Tr>
              ) : (
                sortedUsers.map((user, i) => (
                  <Table.Tr key={user.id}>
                    <Table.Td><Text fz="sm" c="dimmed">{rowNumber(page, PAGE_SIZE, i)}</Text></Table.Td>
                    <Table.Td><Group gap="xs" wrap="nowrap"><Text fz="sm" fw={500}>{user.firstName} {user.lastName}</Text>{user.adminRequestedAt && <Badge size="xs" color="orange" variant="filled">Admin Request</Badge>}</Group></Table.Td>
                    <Table.Td><Text fz="sm" c="dimmed">{user.email}</Text></Table.Td>
                    <Table.Td><Text fz="sm">{user.phone ?? '-'}</Text></Table.Td>
      ...
      <Group justify="space-between">
        <Text fz="sm" c="dimmed">{total > 0 ? `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total.toLocaleString()} users` : 'No users to display'}</Text>
        {totalPages > 1 && <Pagination total={totalPages} value={page} onChange={setPage} size="sm" color="primary" />}
      </Group>

      <UserDetailDrawer userId={selectedUserId} opened={drawerOpened} onClose={closeDrawer} onStatusChange={fetchUsers} />
    </Stack>
  )
}
