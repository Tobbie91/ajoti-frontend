...
export const PHONE_RULES: Record<string, { nationalLength: number; example: string }> = {
...
export function isAdultDob(value: Date | null, minimumAge = 18, today = new Date()): boolean {
...
export function validatePersonName(value: string, label: string): string | undefined {
  ...
  if (normalized.length < 2 || normalized.length > 30) return `${label} must be between 2 and 30 characters.`
  if (!PERSON_NAME_REGEX.test(normalized)) return `${label} can only contain letters, spaces, hyphens, or apostrophes.`
  return undefined
}

export function validatePhone(value: string): string | undefined {
  if (!value) return 'Phone number is required.'
  const code = Object.keys(PHONE_RULES).find((candidate) => value.startsWith(candidate))
...
export function normalizePhoneForComparison(value: string): string {
  ...
  if (digits.startsWith('234')) return `+234${digits.slice(3).replace(/^0+/, '')}`
