...
export function createApiClient(config: ApiClientConfig): ApiClient {
  ...
  function showBankRemovalVerificationModal(
    message?: string,
  ): Promise<string | null> {
    return new Promise((resolve) => {
      const existing = document.getElementById(
        "ajoti-bank-removal-verification",
      );
      existing?.remove();
      ...
      title.style.cssText =
        "margin:0;font-size:18px;line-height:1.3;font-weight:700;color:#111827";
      ...
      close.textContent = "×";
      close.style.cssText =
        "border:0;background:transparent;color:#4B5563;font-size:28px;line-height:1;cursor:pointer;padding:0 2px";
      ...
      copy.style.cssText =
        "margin:0 0 28px;color:#374151;font-size:16px;line-height:1.55";
      if (email) {
        ...
        copy.append(
          strong,
          ". Enter it below to confirm removal of this bank account.",
        );
      } else {
        copy.textContent =
          message ??
          "We’ve sent a 6-digit verification code to your email. Enter it below to confirm removal of this bank account.";
      }

      const inputsWrap = document.createElement("div");
      inputsWrap.style.cssText =
        "display:flex;justify-content:center;gap:12px;margin:0 0 34px";
      ...
      error.style.cssText =
        "display:none;margin:-20px 0 18px;color:#DC2626;font-size:13px;text-align:center";
      ...
      back.style.cssText =
        "display:block;margin:14px auto 0;border:0;background:transparent;color:#7C8593;font-size:14px;font-weight:600;cursor:pointer;padding:4px 12px";
  ...
  async function verifiedBankAccountRemoval<T>(path: string): Promise<T> {
    ...
    const requestResult = await authRequest<{
      success: boolean;
      data?: { message?: string };
    }>(requestPath, {
      method: "POST",
    ...
    const otp = await showBankRemovalVerificationModal(
  ...
  async function authRequest<T>(
    ...
    const { response: res, data } = await performAuthenticatedRequest<T>(
      path,
      options,
    );
    ...
    if (!isRefreshing) {
      ...
      try {
        ...
        const { response: retryRes, data: retryData } =
          await performAuthenticatedRequest<T>(path, options, newToken);
        if (!retryRes.ok) {
          throw apiErrorFromResponse(retryData, retryRes.status);
        }
      ...
      refreshQueue.push((newToken) => {
        performAuthenticatedRequest<T>(path, options, newToken)
          .then(({ response, data: queuedData }) => {
            if (!response.ok) {
              reject(apiErrorFromResponse(queuedData, response.status));
              return;
            }
